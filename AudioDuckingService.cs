...
/// <summary>
/// Temporarily lowers the per-session volume of other applications while dictation is active.
/// Every session is restored to its individual pre-dictation volume afterwards.
/// </summary>
internal sealed class AudioDuckingService : IDisposable
{
    ...
    /// <summary>
    /// Picks up playback sessions which appeared after dictation started.
    /// This is called by a UI timer, keeping all Core Audio access on the STA thread.
    /// </summary>
    public void Refresh()
    {
        if (_active)
    ...
    private void ApplyToNewSessions()
    {
        ...
        {
            // Dictation must continue even when an individual sound driver rejects session access.
