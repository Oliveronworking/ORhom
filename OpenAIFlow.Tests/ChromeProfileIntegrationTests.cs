...
public sealed class ChromeProfileIntegrationTests
{
    ...
    public async Task CorrelatedSharedUserDataProfileLaunchLeavesExistingChromeWindowsUntouched()
    {
        ...
        var sentinelMarker = $"https://openai-flow.invalid/integration-sentinel/{Guid.NewGuid():D}";
...
internal sealed class ChromeIntegrationFactAttribute : FactAttribute
{
    ...
    {
        if (!string.Equals(
                Environment.GetEnvironmentVariable("OPENAIFLOW_RUN_CHROME_INTEGRATION"),
                "1",
                StringComparison.Ordinal))
        {
            Skip = "Set OPENAIFLOW_RUN_CHROME_INTEGRATION=1 to run the isolated real-Chrome test.";
        }
    }
}
